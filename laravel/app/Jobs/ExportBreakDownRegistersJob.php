...
use App\Exports\BreakDownRegistersExport;
...
class ExportBreakDownRegistersJob implements ShouldQueue
{
    ...
    {
        $breaks = $this->fetchBreakDowns();

        $filename = 'breakdown_register_' . now()->format('Ymd_His') . '.xlsx';
        ...
        $excel = Excel::raw(new BreakDownRegistersExport($breaks), \Maatwebsite\Excel\Excel::XLSX);
        ...
        DownloadedReport::create([
            ...
            'file_name' => $filename,
            'report_name' => 'Break Down Register',
        ]);
    }

    private function fetchBreakDowns()
    {
