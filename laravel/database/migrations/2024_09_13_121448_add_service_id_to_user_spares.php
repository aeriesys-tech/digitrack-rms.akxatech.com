...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('user_spares', function (Blueprint $table) {
            $table->unsignedBigInteger('service_id')->nullable()->after('spare_id');
            $table->foreign('service_id')->references('service_id')->on('services');
            $table->unsignedBigInteger('asset_zone_id')->nullable()->after('service_id');
            $table->foreign('asset_zone_id')->references('asset_zone_id')->on('asset_zones');
            $table->string('service_cost')->nullable()->after('asset_zone_id'); 
        });
    }
