    ...
    <div class="sidebar sidebar-dark">
        ...
        <div id="sidebarMenu" class="sidebar-body">
            ...
            <div class="nav-group" :class="{show:showType}" @click="showTab('TypeParameters')" v-if="permission('TypeParameters')">
                ...
                <ul class="nav nav-sidebar">
                    ...
                    <li class="nav-item" v-can="'service_types.view'">
                        <router-link to="/service_types" v-bind:class="{ active: $route.path === '/service_types' }" class="nav-link"><i class="ri-layout-fill"></i> <span>Service Type</span></router-link>
                    </li>  
            ...
            <div class="nav-group" v-if="permission('Registers')" :class="{show:showRegister}" @click="showTab('Registers')">
                ...
                <ul class="nav nav-sidebar">
                    <li class="nav-item" v-can="'userActivities.view'">
                        <router-link v-bind:class="{ active: $route.path === '/activities' || $route.name === 'Activity.Create' || $route.name === 'Activity.Edit'}" to="/activities" class="nav-link"><i class="ri-list-check"></i><span>Activity Register</span></router-link>
                    </li>
                    <li class="nav-item" v-can="'userServices.view'">
                        <router-link v-bind:class="{ active: $route.path === '/user_services' || $route.name === 'UserServices.Create' || $route.name === 'UserServices.Edit'}" to="/user_services" class="nav-link"><i class="ri-flow-chart"></i><span>Service Register</span></router-link>
                    </li> 
                    <li class="nav-item" v-can="'userChecks.view'">
                        <router-link v-bind:class="{ active: $route.path === '/user_checks' || $route.name === 'UserChecks.Create' || $route.path === 'UserChecks.Edit'}" to="/user_checks" class="nav-link"><i class="ri-survey-line"></i><span>Check Register</span></router-link>
                    </li>                    
                    <!-- <li class="nav-item" v-can="'assets.view'">
                        <router-link to="/assets" class="nav-link"><i class="ri-briefcase-fill"></i><span>Asset</span></router-link>
                    </li>          -->
                </ul>
            </div> 
            ...
            <div class="nav-group" v-if="permission('UserManagement')" :class="{show:showUser}" @click="showTab('UserManagement')">
                ...
                <ul class="nav nav-sidebar">
                    ...
                    <li class="nav-item" v-can="'users.view'">
                        <router-link to="/users" v-bind:class="{ active: $route.path === '/users' || $route.name === 'Users.Create' || $route.path === 'Users.Edit'}" class="nav-link"><i class="ri-user-line"></i> <span>Users</span></router-link>
                    </li>
            ...
            <div class="nav-group show">
                ...
                <li class="nav-item">
                    <a href="javascript:void(0)" class="nav-link">
                    <i class="ri-store-3-fill"></i> <span>Activity Report</span>
                    </a>
                </li>
                </ul>
            </div>
...
export default {
        ...
        methods: {
            ...
            showTab(tab, name){
                ...
                if(tab == '/clusters' || tab == '/plants' || tab =='/section' || tab =='/frequency' || tab =='/department'){
                    this.color='white !important';
                    this.showLinage = !this.showLinage;
                }else if(tab == '/equipment_types' || tab == '/asset_types' || tab =='/service_types' || tab =='/spare_types' || tab =='/reasons'){
                ...
                }else if(tab == '/checks' || tab == '/spares' || tab =='/services'){
                ...
                }else if(tab == '/activities' || name == 'Activity.Create' || name=='Activity.Edit' || tab == '/user_services' || name == 'UserServices.Create' || name=='UserServices.Edit' || tab == '/user_checks' || name == 'UserChecks.Create' || name=='UserChecks.Edit'){
                ...
                if (permissions && permissions.length != 0) {
                    ...
                    if (ability == "LineageParameters") {
                        ...
                        permission = permissions.filter(function (el) {
                            return (
                                el.ability.ability == "clusters.view" ||
                                el.ability.ability == "plants.view" ||
                                el.ability.ability == "sections.view"
                            );
                        });
                    } 
                    else if (ability == "TypeParameters") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "equipment_types.view" || 
                    ...
                    if (ability == "MotorParameters") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "voltages.view" || 
                    ...
                    if (ability == "ListParameters") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "checks.view" || 
                            el.ability.ability == "spares.view" ||
                            el.ability.ability == "services.view"
                        });
                    }
                    if (ability == "Masters") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "equipment.view" || 
                            el.ability.ability == "assets.view"
                        });
                    }
                    if (ability == "UserManagement") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "users.view" || 
                            el.ability.ability == "roles.view" || 
                            el.ability.ability == "permissions.view"
                        });
                    }
                    if (ability == "Registers") {
                        permission = permissions.filter(function (el) {
                            return el.ability.ability == "userActivities.view" ||
                            el.ability.ability == "userServices.view" ||
                            el.ability.ability == "userChecks.view"
