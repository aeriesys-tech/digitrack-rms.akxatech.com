    ...
    <div class="">
        ...
        <div class="row g-2">
                ...
                <form @submit.prevent="submitForm()">
                    <div class="card card-one">
                        <div class="card-header d-flex justify-content-between">
                            <h6 class="card-title" v-if="status">Add Data Source Type</h6>
                            <h6 class="card-title" v-else>Update Data Source Type</h6>
                        </div>
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-md-12">
                                    <label class="form-label">Data Source Type Code</label><span class="text-danger"> *</span>
                                    <input type="text" placeholder="Enter Data Source Type Code" class="form-control" :class="{ 'is-invalid': errors.data_source_type_code }" v-model="data_source_type.data_source_type_code" ref="data_source_type_code"/>
                                    <span v-if="errors.data_source_type_code" class="invalid-feedback">{{ errors.data_source_type_code[0] }}</span>
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label">Data Source Type Name</label><span class="text-danger"> *</span>
                                    <input type="text" placeholder="Enter Data Source Type name" class="form-control" :class="{ 'is-invalid': errors.data_source_type_name }" v-model="data_source_type.data_source_type_name"/>
                                    <span v-if="errors.data_source_type_name" class="invalid-feedback">{{ errors.data_source_type_name[0] }}</span>
                                </div>
                            </div>
                        </div>
            ...
            <div :class="column">
                <div class="card card-one">
                    <div class="card-header d-flex justify-content-between">
                        <h6 class="card-title">Data Source Types</h6>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-sm text-nowrap table-striped table-bordered mb-0">
                                <thead>
                                    <tr class="" style="background-color: #9b9b9b; color: white;">
                                        ...
                                        <th @click="sort('data_source_type_code')">Data Source Type Code
                                            <span>
                                                <i v-if="meta.keyword=='data_source_type_code' && meta.order_by=='asc'" class="ri-arrow-up-line"></i>
                                                <i v-else-if="meta.keyword=='data_source_type_code' && meta.order_by=='desc'" class="ri-arrow-down-line"></i>
                                ...
                                <tbody>
                                    <tr v-if="data_source_types.length==0">
                                        <td colspan="6" class="text-center">No records found</td>
                                    </tr>
                                    <tr v-for="data_source_type, key in data_source_types" :key="key">
...
export default {
    ...
    methods: {
        submitForm() {
            let vm = this;
            if (vm.status) {
                vm.addDataSourceType();
            } else {
                vm.updateDataSourceType();
            }
        },
        index() {
            let vm = this;
        ...
        deleteDataSourceType(data_source_type) {
            let vm = this;
            data_source_type.status = data_source_type.status == 1 ? 0 : 1;
            vm.$store
                .dispatch("post", { uri: "deleteDataSourceType", data: data_source_type })
        ...
        editDataSourceType(data_source_type) {
        ...
        discard() {
            let vm = this;
            vm.data_source_type.data_source_type_code = "";
            vm.data_source_type.data_source_type_name = "";
            vm.$refs.data_source_type_code.focus();
            vm.errors = [];
            vm.status = true;
            vm.index();
        },
