<template>
                            ...
                            <div class="table-responsive table-responsive-sm">
                                <table class="table table-sm text-nowrap table-striped table-bordered mb-0">
                                    <tr style="background-color:#9b9b9b;color:white;">
                                            ...
                                            <th class="text-center" v-can="'assetParameters.delete'">Status</th>
                                            <th class="text-center" v-can="'assetParameters.update'">Actions</th>
                                        </tr>
                                    <tbody>
                                        <tr v-for="assetparameter, key in assetparameters" :key="key">
                                            <td class="text-center">{{ meta.from + key }}</td>
                                            <td>{{assetparameter.field_name}}</td>
                                            <td>{{assetparameter.display_name}}</td>
                                            <td>{{assetparameter.field_type}}</td>
                                            <td>{{assetparameter.field_values}}</td>
                                            <td>{{assetparameter.field_length}}</td>
                                            <!-- <td v-if="assetparameter.is_required">Yes</td> -->
                                            <td>{{assetparameter?.is_required==1 ?'Yes' : 'No'  }}</td>
                                            <td>
                                                <span v-for="asset_parameter_type, key in assetparameter.asset_parameter_types" :key="key">{{ asset_parameter_type?.asset_type?.asset_type_name }}, </span>
                                            </td>
                                            <td class="text-center" v-can="'assetParameters.delete'">
                                                <div class="form-switch" >
                                                    <input class="form-check-input"  type="checkbox" role="switch" :id="'assetparameter' + assetparameter.asset_parameter_id" :checked="assetparameter.status" :value="assetparameter.status" @change="deleteAssetParameter(assetparameter)" />
                                                    <label class="custom-control-label" :for="'user' + assetparameter.asset_parameter_id"></label>
                                                </div>
                                            </td>
                                            <td class="text-center" v-can="'assetParameters.update'">
                                                <a href="javascript:void(0)" class="text-success" v-if="assetparameter.status" 
                                                    @click="editAssetParameter(assetparameter)"><i class="ri-pencil-line fs-18 lh-1"></i></a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            next((vm) => {
                if(from.name != 'AssetParameters.Edit'){
        ...
        methods: {
            index() {
                ...
                this.$store.dispatch('post', { uri: 'paginateAssetParameters', data: vm.meta })
                    .then(response => {
                        loader.hide();
                        this.assetparameters = response.data.data;
